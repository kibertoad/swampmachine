package net.kiberion.slick2d.util.pathfinding;

import java.util.ArrayList;
import java.util.LinkedList;
import java.util.List;

import net.kiberion.slick2d.util.pathfinding.heuristics.ClosestHeuristic;
...
public class AStarPathFinder implements PathFinder, PathFindingContext {
	...
	/** True if you have not to interlap with diagonal walls while moving */
	 ...
	 */
	public AStarPathFinder(TileBasedMap map, int maxSearchDistance, boolean allowDiagMovement, boolean strictDiagMovement) {
		this(map, maxSearchDistance, allowDiagMovement, new ClosestHeuristic(), strictDiagMovement);
	}
	...
	public AStarPathFinder(TileBasedMap map, int maxSearchDistance, 
						   boolean allowDiagMovement, AStarHeuristic heuristic, boolean strictDiagMovement) {
		this.heuristic = heuristic;
		this.map = map;
		this.maxSearchDistance = maxSearchDistance;
		this.allowDiagMovement = allowDiagMovement;
		this.strictDiagMovement = strictDiagMovement;
	...
	protected boolean isValidLocation(Mover mover, int sx, int sy, int x, int y) {
		...
		if ((!invalid) && ((sx != x) || (sy != y))) {
			...
			if ((invalid == false) && (strictDiagMovement) && (x != sx && y != sy)) {
			invalid = map.blocked(this, x, sy) || map.blocked(this, sx,  y);     
			}
		}
